...
class HiggsAudioServeEngine:
    ...
    def _prepare_inputs_with_audio_context(
        ...
        """Prepare inputs with explicit audio context for voice continuity.

        This mirrors generation.py's approach where context_audio_ids contains
        both reference audio tokens and previously generated audio tokens.
        """
        ...
        if context_audio_ids:
            ...
            audio_ids_start = torch.cumsum(
                torch.tensor([0] + [ele.shape[1] for ele in context_audio_ids], dtype=torch.long),
                dim=0,
            )
        else:
            audio_ids_concat = None
    ...
    async def generate_delta_stream_with_context(
        ...
        """Generate audio with voice continuity using accumulated context.

        This method mirrors generation.py's approach for maintaining consistent voice
        across multiple generations by reusing previously generated audio tokens as context.

        Args:
            messages: Initial context messages (includes system message with speaker profile)
            audio_ids: Reference audio tokens (empty list for profile-only mode)
            text: Current text chunk to synthesize
            generated_audio_ids: Previously generated audio tokens for context
            generation_messages: Previous user/assistant message pairs for context
            generation_chunk_buffer_size: Rolling buffer size for context (default 2)
            max_new_tokens: Maximum new tokens to generate
            temperature: Sampling temperature
            top_k: Top-k sampling parameter
            top_p: Top-p (nucleus) sampling parameter
            ras_win_len: RAS window length (set to None or <=0 to disable)
            ras_win_max_num_repeat: Maximum RAS window repeats
            seed: Random seed for generation
            stop_strings: Strings that stop generation
            force_audio_gen: Force audio token generation (default True)

        Yields:
            HiggsAudioStreamerDelta with audio_tokens during streaming.
            Final delta has finish_reason="stop" and audio_tokens containing the
            full audio_out_ids tensor (after delay pattern reversal) for use as
            context in subsequent generations.
        """
        if stop_strings is None:
            stop_strings = ["<|end_of_text|>", "<|eot_id|>"]
        if ras_win_len is not None and ras_win_len <= 0:
        ...
        with torch.no_grad():
            inputs = self._prepare_inputs_with_audio_context(
                chatml_sample,
                context_audio_ids,
            ...
            # Accumulate audio tokens for context
            audio_tokens_list = []
            ...
            # Process accumulated audio tokens for use as context in next generation
            if audio_tokens_list:
                # Stack tokens: each delta.audio_tokens has shape [num_codebooks]
                # Result shape: [num_codebooks, seq_len]
                audio_out_ids = torch.stack(audio_tokens_list, dim=1)

                # Apply delay pattern reversal (same as generation.py lines 355-357)
                if self.model.config.use_delay_pattern:
                    audio_out_ids = revert_delay_pattern(audio_out_ids)

                # Clip and trim (same as generation.py line 357)
                audio_out_ids = audio_out_ids.clip(0, self.audio_codebook_size - 1)[:, 1:-1]

                # Yield final delta with accumulated audio_out_ids for caller to use as context
                yield HiggsAudioStreamerDelta(
                    finish_reason="stop",
                    audio_tokens=audio_out_ids,
                )
            else:
